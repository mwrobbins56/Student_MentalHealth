...
3. **Addiction level ceiling in D2** Finding: Just over half of the phone addiction dataset, 1,524 of 3,000 students, scores a perfect 10 out of 10 on addiction level. That ceiling flattens comparisons run against the column itself. Rerunning the same age, gender, and phone usage purpose comparisons against phone_checks_per_day, a column with no ceiling, showed the ceiling was partly to blame, age and phone usage purpose separate more once the cap is removed. But the gaps stay modest either way, none of the groups differ by more than about 6 percent of the average, so the flat pattern is not purely a measurement artifact.
...
```
Overall, this analysis found more flat and inconsistent patterns than confirmed ones. The clearest finding is a data quality issue, D2's addiction_level column has a hard ceiling that limits what it can show. Swapping in an uncapped column showed that ceiling was only partly responsible for the flat comparisons, real but modest differences by age and phone usage purpose exist underneath it. The findings here seem very similar to the findings presented in another student's project (Smartphone Usage Analysis by Melissa Haertzen), which I ultimately used as a template in the presenting this project's conclusion: that the value here is in the database design and SQL query practice, not in proving cause and effect from synthetic data.
```
